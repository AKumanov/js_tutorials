...
export const editView = (ctx) => {
    const submitHandler = (e) => {
        ...
        if(!(validators.validateForm([name, imgUrl, price, releaseDate, artist, genre, description]))) {
            alert('Form is not valid');
            return;
        } 

        albumService.editAlbum(ctx.params.id, {name, imgUrl, price, releaseDate, artist, genre, description})
            .then(album => console.log(album));
        ctx.page.redirect(`/albums/${ctx.params.id}`);
    }
    albumService.getOneAlbum(ctx.params.id)
        .then(album => {
